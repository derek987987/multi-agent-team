...
- Routes work to CTO, PM, implementation, or validation agents through `agent-control/handoffs.md` and agent logs.
- Creates queued inbox routes that the route watcher dispatches to auto-launched Codex agents.
- Uses `agent-control/company/agent-profiles.jsonl` to select the right agent for work.
- Creates and closes meeting records for cross-role discussion before tasking.
- Records human approvals and accepted risks in `agent-control/approvals.jsonl`.
...
Can edit:
- `agent-control/brief.md` for clear product/spec updates
- `agent-control/decisions.md` for routing and tradeoff records
- `agent-control/task-board.md` for simple, obvious task updates
...
- Records major tradeoffs in `agent-control/decisions.md`.
...
- Updates `agent-control/product-requirements.md`.
...
- Updates `agent-control/research-notes.md`.
...
- Updates `agent-control/design-notes.md`.
...
- Report cross-role needs through `agent-control/handoffs.md` or a routed inbox entry instead of asking the human.
...
Default ownership:
- data/model/migration paths assigned in `agent-control/ownership/data.paths`
...
Default ownership:
- platform paths assigned in `agent-control/ownership/devops.paths`
...
- Updates `agent-control/qa-plan.md`.
- Coordinates with validation, PM, and implementation agents.

Default ownership:
- QA/test paths assigned in `agent-control/ownership/qa.paths`
...
- Updates `agent-control/performance-report.md`.
- Coordinates with frontend, backend, data, DevOps, QA, validation, and CTO.

Default ownership:
- performance paths assigned in `agent-control/ownership/performance.paths`
...
Responsibilities:
- Run checks from `agent-control/quality-gates.md`.
...
- Writes `agent-control/review-report.md`.
...
- Writes `agent-control/security-report.md`.
...
- Updates `agent-control/release-notes.md`.
- Coordinates with product, PM, QA, DevOps, and validation.

Default ownership:
- documentation paths assigned in `agent-control/ownership/docs.paths`
