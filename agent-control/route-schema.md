...
2. `scripts/watch-routes.sh` normally runs in the control window, applies agent-session recovery, applies stale-route recovery, and calls `scripts/dispatch-routes.sh`.
3. `scripts/heartbeat-routes.sh` can run a heartbeat-style pass over the structured queue, runs agent-session and stale-route recovery by default, and delegates dispatch. Use `--no-recover-stale`, `AGENT_TEAM_AUTO_RECOVER_AGENTS=0`, or `AGENT_TEAM_AUTO_RECOVER_STALE=0` only for diagnostic scans that must not mutate recovery state.
