...
if [ "$#" -lt 1 ]; then
  printf "Usage: %s <route-id> [actor] [summary]\n" "$(basename "$0")" >&2
  exit 1
fi

ROUTE_ID="$1"
ACTOR="${2:-unknown}"
SUMMARY="${3:-Completed route $ROUTE_ID}"
...
printf '{"route_id":"%s","status":"done","actor":"%s","summary":"%s"}\n' \
  "$(json_escape "$ROUTE_ID")" "$(json_escape "$ACTOR")" "$(json_escape "$SUMMARY")" >> "$ROOT/.agents/state/routes.jsonl"
